...
export default function JournalEntry(props) {
    ...
    const [editEntry, setEditEntry] = useState({res1: props.res1, res2: props.res2, res3: props.res3, share: props.share});
    ...
    const handleEmail = async () => { 
        try {
          const response = await axios.post(`https://mighty-mini-minds-backend.onrender.com/sendemail/${editId}`);
    ...
    <> {isEditing ?(
        ...
        <div className = 'flex flex-col justify-start items-start bg-skin-input mx-4 mb-4 p-2 rounded-lg shadow-md'>
            ...
            <div className='flex flex-row item-start justify-end w-full gap-5 mt-4 mb-2'>
                ...
                <button aria-label="delete-button" onClick={()=>handleDeleteEntry(props.id)} className='rounded-md w-10 h-10 bg-skin-secondary mr-2 text-white transition-colors duration-300 ease-in-out transform hover:scale-125'><FontAwesomeIcon icon={faTrash}  /></button>
            </div>
